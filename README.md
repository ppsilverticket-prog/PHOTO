...
- 지우개: exemplar-based inpainting (Criminisi). 모델 파일 없이 순수 Dart로
  동작해 웹 미리보기에서도 바로 쓸 수 있다. 붓 크기 조절, 붓질 단위 되돌리기
- 화질 개선: Lanczos-3 + 반복 역투영(back-projection) + 헤일로 억제 적응 샤픈.
  2배/4배, 선명도 조절, 전/후 비교. 저장 시 원본 해상도에 적용되며
  출력 긴 변 3200px 상한(시간·메모리 보호, 초과 시 배율 자동 축소)
...
다음 마일스톤: **M5 폴리싱 → M6 서버 기능 + 결제**.
지우개는 LaMa, 화질 개선은 Real-ESRGAN을 붙이면 품질이 한 단계 올라가며,
둘 다 지금 인터페이스 뒤로 백엔드만 교체하면 된다 (신경망 모델은
없는 디테일을 "생성"하므로 이 시점부터 크레딧 과금 명분이 생긴다).
...
- `lib/core/upscale/` — 화질 개선. Lanczos-3 분리형 리샘플 위에 반복
  역투영("결과를 축소하면 원본과 같아야 한다"를 강제)을 얹고, Lanczos
  링잉은 원본 3x3 국소 범위 클램프로, 샤픈 헤일로는 이웃 최소/최대
  클램프로 억제한다. 전부 테스트로 고정되어 있다.
