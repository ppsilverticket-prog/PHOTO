...
**M6 결제 구조 완료** — 무료/프리미엄 경계 + 페이월 (결제 백엔드는 교체형):

- 기획서의 경계 그대로: 지우개 일 3회 · 화질 개선 2배 일 3회/4배 프리미엄 ·
  프리미엄 필터 6종 잠금 · 무료 저장 시 워터마크
- `EntitlementService`가 경계의 단일 진실 공급원. 일일 카운터는 날짜가
  바뀌면 자동 리셋, 구독 상태는 재시작해도 유지
- 페이월: 혜택/플랜(월·연) UI 완성. 결제는 `BillingService` 인터페이스 뒤에
  있어 지금은 **로컬 테스트 모드**(화면에 명시)로 동작하고, 스토어 출시 시
  RevenueCat 구현체로 교체하면 된다
- 구매 즉시 모든 게이트가 실시간 해제 (ChangeNotifier)
