...
`pod install`이 CocoaPods 없다고 하면: `sudo gem install cocoapods`
...
2. **얼굴** — 셀피를 열어 얼굴이 인식되는지, "자연스럽게" 프리셋과
   피부결·눈 크기·얼굴 슬림 슬라이더
3. **필터** — 19종 썸네일과 강도 슬라이더
4. **보정** — 밝기/대비/채도 등 7종 슬라이더가 **끊김 없이 즉시** 반응하는지
   (여기가 GPU 셰이더 경로다)
5. 사진을 **길게 눌러** 원본과 비교
6. **저장** 후 사진 앱에서 결과 확인 — 화면에서 본 것과 일치하는지
